 ...
 * 解决Fragment 保存fragment页面的状态
 * iewPager有5个page，刚打开的时候，会加载page1和page2，
 * 我们手动切换到page2的时候，会加载page3，切换到page3的时候，加载page4的同时会destory掉page1，
 * 采用隐藏和再显示的方式 onPause，和onResume交替使用
 * 赵宇环 测试1234
 */
public class FragmentViewPagerAdapter extends PagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (null==fragment.getView().getParent()) {
            container.addView(fragment.getView()); // 为viewpager增加布局,一定要有getParent
        }
