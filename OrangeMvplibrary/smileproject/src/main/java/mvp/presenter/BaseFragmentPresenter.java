...
public class BaseFragmentPresenter<T extends IBaseView> {
    ...
    public static class BaseHandler extends Handler {
        private final WeakReference<BaseFragmentPresenter> reference;

        public BaseHandler(BaseFragmentPresenter ref) {
            Log.e("BasePresenter", "检测Hander中持有的对象类型----->" + ref.getClass().getSimpleName());
            reference = new WeakReference<BaseFragmentPresenter>(ref);

        }

        @Override
        public void handleMessage(Message msg) {
            BaseFragmentPresenter presenter = reference.get();
            if (presenter != null) {
    ...
    public void attchView(T mBaseView) {
        Log.d(TAG,"hhhh mBase View : "+mBaseView);
